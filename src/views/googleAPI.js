...
class Castjs {
  constructor(t = {}) {
    (t.joinpolicies &&
      -1 !==
        ["tab_and_origin_scoped", "origin_scoped", "page_scoped"].indexOf(
          t.joinpolicy
        )) ||
  ...
  _init(t = 0) {
    if (
      ...
      return t++ > 20
        ? this.trigger("error", "Casting is not supported in this browser")
    ...
    this._player.isMediaLoaded &&
      setTimeout(() => {
        if (this._player.mediaInfo) {
          ...
          (this.subtitles = []),
          (this.volumeLevel = this.volumeLevel = Number(
